...
rm -rf "$ISOLATED_ROOT/CI"
mkdir -p "$ISOLATED_ROOT/WORKSPACE-GUARD" "$ISOLATED_ROOT/CI"
...
tar --exclude=.git --exclude=.venv --exclude=node_modules --exclude=.boot-linux -cf "$ISOLATED_ROOT/workspace-ci.tar" -C "$PROJECTS_ROOT/CI" .
tar --no-same-owner -xf "$ISOLATED_ROOT/workspace-ci.tar" -C "$ISOLATED_ROOT/CI"
