// git-guard/build.rs - policy compiler for the privileged Git guard.
//
// Reads the git_guard_* + shared_* policy YAMLs from ../config, validates
// the policy matrix against the subcommand categories, and emits
// OUT_DIR/guard_config.rs. The privileged package has its own lockfile and
// no path dependency on the top-level tools package; this build script is
// the only build-time consumer of the git-guard policy files.
...
fn main() {
    ...
    let config_dir = Path::new(&manifest).join("..").join("config");
