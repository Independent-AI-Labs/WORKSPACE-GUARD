...
`config/shared_host_profiles.yaml` (`hostname -s` to class). See
...
compile-time baked from `config/binary_guard_policy_rules.yaml` and
...
defined in `config/shared_locked_paths.yaml`, which declares four categories:
