...
- `src/binary_guard/policy_types.rs` -- `BinaryPolicy`, `RejectRule`,
  `RejectKind`, `find_policy`. Hand-written, IDE-supported, no string-built
  structs.
- `src/binary_guard/main.rs` -- the guard runtime binary. `include!`s the
  generated table.
...
|------|-----------|--------|-------|
| `config/binary_guard_policy_rules.yaml` | hand-maintained, committed | human | host-INdependent rules covering the FULL GTFOBins universe |
| `res/binary-lock.yaml` | generated, committed snapshot | `make sync-gtfobins` | per-ON-host view: full GTFOBins set + `contained:` flag |

`config/binary_guard_policy_rules.yaml` is the catalog of rules. Every GTFOBins
...
4. Joins the full GTFOBins set against `config/binary_guard_policy_rules.yaml`:
...
### 3.2 Rule file schema (`config/binary_guard_policy_rules.yaml`)
...
```yaml
# config/binary_guard_policy_rules.yaml -- full GTFOBins coverage, host-independent.
...
live in `src/binary_guard/policy_types.rs`; the guard runtime lives in
`src/binary_guard/main.rs` (section 5).
...
| File | Role | Generated? |
|------|------|-----------|
| `src/binary_guard/policy_types.rs` | `BinaryPolicy`, `RejectRule`, `RejectKind` structs, `find_policy` function. Hand-written under full IDE support. | No |
| `src/binary_guard/main.rs` | The guard runtime: `decide`, `check_arg_validate`, `build_sanitized_env`, `execve_real`, `log_block`, `main`. `include!`s the generated table. | No |
| `src/binary_guard/tests.rs` | Unit tests for policy decision logic + env sanitisation. | No |
...
| `config/binary_guard_policy_rules.yaml` | Hand-maintained rule catalog (full GTFOBins coverage). Input to sync. | No |
