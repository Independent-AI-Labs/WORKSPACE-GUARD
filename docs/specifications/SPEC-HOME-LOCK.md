...
state is the baseline; the report is the latest audit).
...
State file format (`res/home-lock-state.yaml`):
...
State is committed to the repo (it is the baseline for
`home-drift-check` and the rollback map for `uninstall-home-lock`).

### 4.3 uninstall-home-lock

```
Usage: scripts/uninstall-home-lock [--dry-run]
Reverse home lock per res/home-lock-state.yaml (SPEC-HOME-LOCK 4.3).
...
Report file format (`res/home-drift-report.yaml`):

```yaml
# Auto-generated by scripts/home-drift-check on 2026-07-09T22:00:00Z
# Baseline snapshot: res/home-lock-state.yaml
drift:
